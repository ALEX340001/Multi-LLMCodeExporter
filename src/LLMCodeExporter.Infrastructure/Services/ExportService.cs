    ...
    private async Task<ExportResult> ExportProjectAsync(ProjectInfo projectInfo, ExportSettings settings)
{
    ...
    try
    {
        ...
        {
            var jsonFormatter = new Formatters.JsonFormatter();
            string jsonContent = jsonFormatter.Format(projectInfo);
            string jsonExtension = "json";
            string jsonFileName = $"code_export_{projectInfo.ProjectName}_{DateTime.Now:yyyyMMdd_HHmmss}.{jsonExtension}";
            string jsonOutputPath = Path.Combine(settings.OutputDirectory, jsonFileName);
            await File.WriteAllTextAsync(jsonOutputPath, jsonContent, Encoding.UTF8);
            result.Success = true;
            result.OutputFilePath = jsonOutputPath;
            result.Message = $"Экспорт завершен успешно: {jsonOutputPath}";
            return result;
        }

        // ========== ОБЫЧНЫЕ ФОРМАТЫ (Markdown / PlainText) ==========
        ...
        using (var writer = new StreamWriter(outputPath, false, new UTF8Encoding(true)))
        {
            ...
            {
                var jsonFormatter = new Formatters.JsonFormatter();
                string json = jsonFormatter.Format(projectInfo);
                await writer.WriteLineAsync();
                await writer.WriteLineAsync("---");
                await writer.WriteLineAsync();
                await writer.WriteLineAsync("<details>");
                await writer.WriteLineAsync("<summary>📦 JSON-представление (структурированные данные)</summary>");
                await writer.WriteLineAsync();
